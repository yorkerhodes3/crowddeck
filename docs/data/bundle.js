    ...
    "epics": [
      ...
      },
      {
        ...
        "stories": [
          ...
          {
            "id": "DJX-26",
            "name": "Attribution, because the licence requires it",
            "size": "S",
            "verdict": null,
            "status": "done",
            "reqs": [
            ...
            "detail": "A compliance gap in the one area this project exists to get right. CC BY and CC BY-SA require attribution as a CONDITION of the licence - not as a courtesy - and cc-licence.js has always built the credit string. Its own comment even says where it belongs: in a venue that means on the screen while the track plays. Nothing ever displayed it. The string was computed and discarded, which is worse than never having it: the information was present and thrown away, so a deck playing CC BY music was in breach of the licence it depends on. Found by grepping the UI for attribution rather than assuming that carrying the field meant showing it - the same way the session cache turned out to be built and unconnected. DONE - credits render live for whatever is loaded on either deck, and accumulate into a session list that copies to the clipboard, because once a set leaves the room the obligation takes the form of a credit under a published mix. Verified end to end in a browser: loading a track showed Psicotropicodelia Music Vol. 1 by Various Artists - CC BY 3.0 with a session count, and the clipboard came back carrying that exact text plus the source. PUBLIC DOMAIN IS NOT CREDITED, DELIBERATELY. CC0 and the Public Domain Mark impose no attribution obligation, so ccAttribution returns null and this keeps the distinction rather than inventing a credit - presenting a legal requirement where none exists misrepresents the licence just as much as omitting a real one. Public domain tracks are labelled instead, and appear in the pasteable list only when explicitly asked for. The text is plain rather than Markdown because the destinations - a stream description, a caption, a forum post - mangle formatting inconsistently, and a credit that arrives as literal asterisks is worse than a plain one."
          },
          {
            "id": "DJX-27",
            "name": "One source abstraction, not two",
            "size": "S",
            "verdict": null,
            "status": "done",
            "reqs": [
            ...
            "detail": "Reported from use: pressing load on a LibriVox row failed with source.tracks is not a function. Reproduced in a browser before touching anything - Pride and Prejudice, exactly as reported. MY BUG, AND THE EXACT ONE DJX-22 EXISTED TO REMOVE. That story converted the deck to the provider contract, but only wired LibriVox through it - the Archive and Openverse kept their LIBRARY objects. The two have different surfaces: libraries expose tracks() and coverArt(), providers expose files() and art(). All three sat in the same variable slot. Search worked because both shapes happen to have search(), so the mismatch was invisible until someone pressed load. Half-converting an abstraction is worse than not converting it: it looks done and fails only on the path nobody exercised. DONE - the deck now goes through providerFor(row) for search, load, artwork and tempo, so there is one shape. files() joined the browser contract because that is where the two worlds genuinely differ - a venue engine takes a single stream URL, a browser needs the file NAME to title the deck and its SIZE for progress - and streamUrl() is now DERIVED from files() rather than implemented separately, so the two can never disagree about which file is playing. Two orphans fell out of the conversion: window.__crowddeck still referenced a librivox binding that no longer existed, which would have been a ReferenceError, and a remember() call was operating on a different instance of OpenverseLibrary than the provider actually reads, so it did nothing. THE GUARD I WROTE FIRST DID NOT WORK. It asserted typeof provider[method] === function for every method the deck calls. Reverting LibriVox to the broken shape left it PASSING, because the base class defines the unimplemented methods as stubs that throw - they are functions whether or not anyone wrote them. Rewritten to check the method was actually OVERRIDDEN, by identity against the base prototype, and then re-verified by reintroducing the bug and watching it fail. Third hollow test caught this session by the same discipline. Also fixed a display fault the repro exposed: LibriVox chapters are Archive slugs like prideandprejudice_30_austen_64kb, which tidied still reads as a filename and looks like a fault on a deck. The provider now names them with the release title, and deliberately does NOT parse a chapter number out of the slug, because the files are ordered by size rather than by chapter so any index shown would be confidently wrong."
          },
          {
            "id": "DJX-28",
            "name": "Only offer records that can actually be played",
            "size": "S",
            "verdict": null,
            "status": "done",
            "reqs": [
            ...
            "detail": "Reported from use, as a follow-on to DJX-27: 'this error is coming up on a lot of the audio readings; haven't checked beyond that'. Took the second half of that sentence as the actual instruction and swept 30 loads across three providers rather than fixing only the one screenshot. 26 of 30 succeeded. THE FOUR FAILURES WERE TWO DISTINCT FAULTS, NEITHER OF THEM THE ONE ALREADY FIXED. Three were Archive netlabel releases published as a ZIP of cover art and a text file with no individual tracks at all - TFN147, TFN120 and TFN124, confirmed against their metadata as playable:0 with only JPEG/Text/ZIP formats. Those items are mediatype:audio and matched every other clause, so they appeared in search results and could only ever produce 'no playable audio in this release' when clicked. The deck was refusing correctly, having offered something it could never play. Fixed at query time with a format facet, because the alternative is a metadata round trip per result - 25 extra requests per search to a charity's servers to discard about one row in twenty. Measured the cost of the filter before adopting it: netlabels 77,003 to 73,218 items (-4.9 percent, the unplayable ones), LibriVox 21,761 to 21,755 (loses 6). All three known-bad items return 0 results under the new query and a known-good release still returns 1. The clause is applied UNCONDITIONALLY rather than beside the licence filter, because LibriVox constructs the library with commercialOnly false and would otherwise have silently lost it. The fourth failure was a transient 'Failed to fetch' on a track that had loaded a minute earlier, so the loader now retries once after 600ms on a network-level failure only - a blip mid-set should not cost a DJ the record, but retrying forever would hide a genuinely dead source. Sweep re-run after the fix: 50 of 50. THE SWEEP ITSELF FAILED FIRST, 0 of 50, AND THAT WAS THE HARNESS NOT THE APP - it passed row objects to files(), which takes an id. Worth recording because a red result from a new instrument is not evidence until the instrument is checked; the giveaway was that every row still carried a real title, so search plainly worked. That debugging exposed a real ambiguity, now fixed: Openverse returned an empty list both for a track it had never seen and for a known row with no audio, so a provider-routing bug was indistinguishable from ordinary bad luck with a search result - which is precisely what made DJX-27 hard to read. An unknown id is now an explicit error naming the provider. Guards verified by breaking them, four times: removing the format clause fails three tests, un-parenthesising it fails the grouping test, and making escapeLucene a pass-through fails the injection test. The injection test also had to be rewritten - it asserted a hard-coded count of eight structural quotes, which my new clause broke simply by existing. A magic number described the query as it stood on the day and forced a security guard to be edited every time a clause was added, which is exactly when you least want one being adjusted to fit the new output. It now compares the quote count against the same query built with a benign term, which states the actual property - a search term cannot add structure - and cannot go stale."
          },
          {
            "id": "DJX-29",
            "name": "The local deck actually starts",
            "size": "S",
            "verdict": null,
            "status": "done",
            "reqs": [
            ...
            "detail": "Found by DRIVING the local deck in a real browser rather than fetching it, while verifying DJX-28. The page rendered perfectly and every single control did nothing. npm start serves clients/deck/index.html, which imports engine-web/, which imports providers/src/provider.js, which imports core/src/policy.js - and core/ was not in the server's SERVED_ROOTS. The 404 aborted the module, so the event listeners at the bottom of the file were never attached. THE WORST SHAPE A BUG CAN TAKE: no error on the page, no broken control, nothing an HTTP check could catch. Fetching the deck returned 200 and so did every module it NAMES - the missing file is imported transitively by a package the deck never mentions. The published demo was unaffected, because tools/build-demo.mjs already resolves imports transitively and had copied policy.js into docs/; only the local server had fallen behind. That is precisely why it survived: the hosted link worked, so nothing pointed at the dev path. Fixed by mounting core/. Guarded by clients/test/deck-imports.test.js, which walks the real import graph from the deck page and asserts every module resolves through the server's own SERVED_ROOTS, parsed out of the server source so the two cannot drift. THE WALK HAD TO BE DONE IN URL SPACE, NOT ON DISK, and the first version was wrong for exactly that reason: the page lives at the URL /deck/index.html but on disk at clients/deck/index.html, so ../engine-web/x.js is /engine-web/x.js to the browser and repoRoot/engine-web/x.js after mounting - NOT clients/engine-web. Walking the disk layout invented files that are never requested and missed the one that 404s. Verified by deleting the core mount and watching the test name /core/src/policy.js, and paired with a second test asserting the walk actually reaches core/, so it cannot pass trivially by matching no imports at all - an empty graph has nothing missing. Two further instrument errors worth recording, both caught before they became conclusions. window.__crowddeck was absent and I briefly read that as the module having failed; it is created on the first gesture, because the AudioContext cannot exist before one. And the load probe waited for a decoded buffer to EXIST, which after the first load is always true, so six different records all reported an identical 165.1 seconds - it was reading the previous track. Waiting for the duration to CHANGE gave six distinct durations and six distinct deck titles. A measurement whose instrument has not been checked is not evidence."
          }
        ]
      }
    ],
